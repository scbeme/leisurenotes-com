...
## Last updated: 2026-08-01 | Session: 19 | Status: Phase 3 complete — Phase 3R next
...
## Phase 3 — Site Build *(Claude Code autonomous)* — ✅ COMPLETE
...
**Pass 3 (derivative generation) — DONE for all 12 remaining projects (Session 18)**, per website-design.md's tooling writeup: homepage thumbnail, cropped 4:3 hero-tile capped at 1600px, capped ~1600px gallery/lightbox derivatives, all WebP, `data/projects.json` thumb updated and verified for every project. Actual page HTML (below) is the remaining work per project. Flag Display Case initially used an interim CAD-render hero pending a customer privacy decision; customer confirmed restoring the original photo as hero + gallery position 1, done same session — see website-design.md and session-notes.md.
...
- [x] 3.10 Flag Display Case — built Session 18. Initially used an interim CAD-render hero pending a customer privacy decision (this is the customer's own tribute to his late father-in-law, a USAF veteran; the exact photo with the same partial redaction is already the large cover image on the customer's own published instructions PDF). Customer confirmed, same session: *"the image 01- is supposed to be the hero image and first image in the gallery"* — real photo restored as both hero and gallery position 1. Hero/thumb derivatives regenerated from the original, a new full-frame gallery derivative added at `01-`, the 3 CAD renders renumbered `02`–`04`, and the hero lightbox target reverted to the original real-photo file. Content complete: full parts list, construction tips, and final-assembly steps from the 8-page instructions PDF. Gallery now shows the real photo plus all 3 CAD renders, 4 images total.
...
- [x] 3.19 Tablesaw Vertical Tenon Jig — built Session 18, the 12th and last remaining project page. Content from live page + 4-page instructions PDF. Cross-linked from/to Flag Display Case (the project this jig was built for), now that both pages exist.

**All 13 project pages are now built. Phase 3's "All 13 Project Pages" checklist is complete.**

**Crayford Focuser folder/slug mismatch — RESOLVED Session 18, then corrected same day.** A full site-wide link verification after all pages were built (190 references across all 16 pages, plus a direct slug-to-folder check) found `data/projects.json`'s `slug` for this project was `crayford-focuser-1-25`, not matching the actual `projects/crayford-focuser/` folder — this had been flagged and deferred since Session 15. Initially fixed by changing the slug to match the folder (`crayford-focuser`). Customer then directed the opposite fix, since the locked sitemap in `website-design.md` had always specified `crayford-focuser-1-25` and the folder was the thing out of step: renamed the folder to `projects/crayford-focuser-1-25/` (via `git mv`, including `_archive/`) and set the slug/thumb path back to `crayford-focuser-1-25` to match. Re-ran the same verification script (16 pages, 235 references, slug-to-folder check) — all clean. Folder, slug, and sitemap now agree.
...
### Assets & Polish
- [x] 3.20 Optimize all images for web — audited every page for `<img src>` still pointing at raw `.png`/`.jpg`/`.jpeg` instead of a WebP derivative. **Found Foosball Table had never been run through the Pass 3 WebP pipeline at all** — it predates that pipeline entirely (built under 2C/2D, before Pass 1/2/3 existed), so its hero + all 34 gallery images were still raw PNGs pulled straight from the WordPress export. Converted all 34 gallery images plus hero/thumb to WebP (`cwebp -q 85`, native resolution — all sources were already under the 1600px cap, no resizing needed). **While converting, discovered and fixed a real, separate bug**: 12 of the underlying raw source PNGs (the hero photo + 11 gallery photos) had their pixel content baked in sideways (90° rotated) — not an EXIF-metadata issue (PNGs here carry none) but the actual pixel data, likely from how these were originally harvested/converted from WordPress pre-dating the current workflow. Confirmed via visual inspection of every one of the 29 gallery photos (not just spot-checks) that this affected exactly 12 images; rotated each 90° clockwise via `sips -r 90` (visually verified correct against the one known-good reference — the pre-existing manually-cropped hero — before batch-applying), regenerated WebP from the corrected pixels, renamed to reflect true post-rotation dimensions (e.g. `768x576` → `576x768`), and archived the broken raw originals to `_archive/`. Also fixed a pre-existing hero-lightbox bug found in the process: the "view full-size" link pointed at a smaller, unrelated crop of the same photo rather than the biggest available version — now points at a corrected, uncropped, full-frame original. `data/projects.json` thumb path updated to match. No other project had this issue (all 12 already went through Pass 3 with visual verification in Session 18). Re-verified site-wide: 0 broken references across all 16 pages.
...
- [x] 3.22 Generate sitemap.xml for SEO — `sitemap.xml` at the repo root lists all 15 canonical URLs (homepage + About + all 13 project pages; 404 excluded) using the `https://leisurenotes.com/` production domain (matches the root-relative internal links already used site-wide, which assume the eventual custom domain rather than the interim `scbeme.github.io/leisurenotes-com/` GitHub Pages URL). No `lastmod`/`changefreq`/`priority` fields — both are optional per the sitemap spec and there's no reliably-accurate modification-date source to populate them honestly. Also created `robots.txt` (none existed) — `Allow: /` plus a `Sitemap:` reference. Both verified valid (XML parses clean, both return HTTP 200 locally).
- [x] 3.23 Add favicon — built Session 18: white "L" monogram on a filled circle in the site's signal blue (`--color-signal`, `#2563eb`), drawn as flat geometric shapes (no font dependency, so it renders identically everywhere). Master source `assets/images/favicon.svg` (circle, transparent corners, for modern browsers' `rel="icon"`), rasterized via `resvg` (installed via `brew install resvg`, since nothing else on this Mac could rasterize SVG) to `favicon-16x16.png` and `favicon-32x32.png` for older browsers. Separate full-bleed square source `assets/images/apple-touch-icon-source.svg` (no circle mask, per Apple's guidance that iOS applies its own corner rounding) rasterized to `apple-touch-icon.png` (180×180). All 4 `<link rel="icon">`/`<link rel="apple-touch-icon">` tags added to every page's `<head>` (homepage, About, 404, all 13 project pages) at the correct relative depth for each. Verified via local server: all icon asset paths return HTTP 200 on every page, full site-wide link check re-run clean (299 refs, 16 pages), and opened a live local-server tab in the browser to confirm the favicon actually renders.
- [x] 3.24 Verify all download links working — went beyond HTTP-200 resolution (already covered by the link-verification script) to actual file-integrity checks on all 21 download files across the 13 project pages: every `.zip` passed `unzip -t` archive integrity testing (0 errors), every `.pdf` has a valid `%PDF-` header and `%%EOF` trailer. All 21 pass. **Noted, not acted on:** two files — Crayford Focuser's SketchUp CAD zip and Fidget Spinner's CAD zip — are ~77MB each, already at the documented "~60-80MB per-file" GitHub watch threshold (see Monitor repo size note, carried in session-notes.md for several sessions). Worth a look before Phase 4 deployment, not a Phase 3 blocker.
  - **Superseded by Session 19's combined-ZIP rebuild** (see website-design.md Q5 amendment and session-notes.md Session 19 Summary): all 13 projects now ship one `<slug>-project-files.zip` each instead of separate per-format buttons/links; the flagged 77MB files are gone — Crayford Focuser's new combined ZIP is 39MB, Fidget Spinner's is 32MB. Re-verified: all 13 new ZIPs pass `unzip -t`, 291 site-wide `src`/`href` references (16 pages) all resolve, 0 broken. No download file exceeds the watch threshold; no project needs GitHub Releases.
- [x] 3.25 Verify mobile responsiveness all pages — **no live browser device-emulation tool available this session**: Claude in Chrome isn't connected, and `safaridriver` (macOS's built-in WebDriver) requires a one-time interactive `sudo safaridriver --enable` that can't be authorized non-interactively in this environment. Did a full CSS/structural code review instead across all 16 pages: viewport meta tag present everywhere; `.hero-grid` (project pages) explicitly stacks below 639px; `.project-grid` (homepage) goes 1→2→4 columns at defined breakpoints; `.browser-controls` stacks below 600px; `.gallery-grid` uses `auto-fill minmax(160px,1fr)` (reflows without needing a breakpoint); no `<table>`, `<pre>`/`<code>`, or fixed-pixel inline widths anywhere (the most common sources of mobile horizontal-scroll bugs); all images are CSS-driven (`width:100%`, `aspect-ratio`, `object-fit:cover`); no viewport-dependent JS. No breaking layout issues found. **Two design-preference notes flagged, not changed unilaterally:** `.spec-grid` stays 2 columns at all widths (wraps to 2 lines on narrow phones for longer values, doesn't break); `.chip` filter buttons compute to ~28-30px tap height, under the ~44px recommended minimum (still fully usable). Both are candidates for a real-device check during Phase 3R rather than confirmed bugs.

---

## Phase 3R — Customer Review & Revisions *(iterative)*
- [x] 3R.1 Customer reviews complete site (Live Preview) — reviewed via the live GitHub Pages testing URL, found 2 site-wide bugs
- [x] 3R.2 Consolidated revision list submitted — 2 bugs reported same session (root-absolute nav links broken on the Pages subpath; hero photo missing from gallery on 8 pages)
- [x] 3R.3 Claude Code implements revisions — both fixed same session: all 16 pages' nav links converted to relative paths (site-title/breadcrumb/footer About), verified live against the actual subpath URL (58 nav links, 0 broken); 8 project pages got a new gallery-position-1 derivative generated from their existing hero photo, matching the other 5 projects' pattern. Re-ran the site-wide link/file-integrity check (299 refs, 0 broken). One residual item found and flagged, not fixed: 404.html's relative-link fix doesn't reliably route back to site root for a trailing-slash 404 path (this site's own URL convention) — see session-notes.md Session 19 follow-up for detail.
