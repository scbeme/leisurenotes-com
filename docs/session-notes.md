...
- **Resume point:** Session 16 (Claude Code terminal) — two workflow/doc updates plus the first real Pass 2 of the image-selection workflow (Bicycle Maintenance Clamp), triggered by the customer dropping a new file into that project's folder between sessions.
  ...
  15. **Biplane Wooden Toy — Pass 2 applied, resequence only, last of the 12 remaining projects** (`projects/biplane-wooden-toy/`): all 3 files already clean, no new/corrupted files. Customer's sequence `01, 03, 02` — no `hero-`. `01` unchanged (already matched); `02`↔`03` was a simple swap, staged through one temp name. Applied the item-14 lesson immediately this time instead of catching it after the fact: checked `data/projects.json`'s thumb for this project *before* finishing up, found it pointed to `03-biplane-render...` (the file that had just become `02-...`), and fixed it in the same pass rather than as a follow-up bug report. **All 12 thumb paths reconfirmed resolving.**
  - **Image review is now complete for all 12 remaining projects.** Pass 3 (derivative generation) and actual page builds are the next phase — not started for any of them yet.
...
- **Still open / needs customer input:**
  ...
  2. **Pass 3 + page builds, all 12 remaining projects** — image review (hero pick + sequence) is now done for every project; the next phase is generating capped derivative sizes (Pass 3, per website-design.md's three-pass workflow) and actually building each project page. Not started yet for any of the 12.
...
- **First action for next session:** Run the new git-status startup check as usual. Image review is complete — next up is deciding how to sequence Pass 3 + page builds across the 12 projects (all at once vs. one at a time, matching how Phase 3's structural audit was already done). Cloudflare Web Analytics is live on all 3 built pages — worth a check of the dashboard for incoming beacon data.
