...
## Last updated: 2026-08-01 | Session: 20 | Phase 3R content-complete, deployment sign-off pending — Jekyll conversion decision next

---

## ⚡ QUICK START — READ FIRST

- **Resume point:** Session 20 (Cowork, customer-facing Phase 3R review) — **Phase 3R content/revision work is complete: customer reviewed the live site at https://scbeme.github.io/leisurenotes-com/, found 6 issues across the session, all fixed by Claude Code and confirmed on real devices (iPhone 14 Pro, iPad).** Full detail in the Session 20 Summary below. **3R.6 (formal deployment sign-off) is explicitly NOT given yet** — customer wants to hold approval until the Jekyll conversion decision (scoped this session, see below) is resolved, since the plan is to deploy once to the custom domain from Jekyll's build output rather than deploy the current hand-coded site now and redeploy again after migrating. **Do not proceed toward Phase 4 until the customer explicitly signs off on 3R.6.** Next session priority: the Jekyll conversion itself (Claude Code, estimated 10–16 hours, one long session) — see "Next session priority: Jekyll conversion" below for the full scoping.
...
- **Tool switch:** Session 20's review/scoping conversation ran in Cowork; the actual bug fixes were implemented in Claude Code per the standing division of labor. Next session (Jekyll conversion) is Claude Code work.
- **Phase 3R content/revision work is now complete** — all 6 issues found and fixed, customer completed a full page-by-page pass across all 16 pages. **Formal deployment sign-off (3R.6) is intentionally still pending** — see resume point above and the Jekyll conversion write-up below.
...
- **First action for next session:** Jekyll conversion (Claude Code) — see "Next session priority: Jekyll conversion" below for the full scoping (recommendation, estimate, sequencing, pre-migration backup step). Take the git tag/branch snapshot of the current hand-coded state **before** starting any conversion work.

### Next session priority: Jekyll conversion (scoped Session 20, Cowork)
- **Recommendation: Jekyll over Eleventy** — specifically for GitHub Pages' native build support (no GitHub Actions workflow needed) and 17+ years of stability, matching the customer's stated priority on reliability/longevity over cutting-edge tooling.
- **Output will be visually/functionally identical to the current live site** — static HTML/CSS/JS either way, no visitor-facing change required.
- **No rework required from the customer** — same raw materials (photos, CAD files, descriptions) as today. Claude Code does the template/front-matter extraction work, since all 13 project pages are already highly uniform in structure.
- **Rough estimate: 10–16 hours of Claude Code working time**, likely one long session (comparable to Session 18's 12-page build day). Customer's own time stays under an hour (approval + a final identical-output review pass).
- **Before starting:** take a git tag/branch snapshot of the current hand-coded state (e.g. `pre-jekyll-migration`) as a full, permanently recoverable backup.
- **Sequencing:** migrate once content is stable (it is, as of this session) → verify output is identical to the live site → **then** deploy to the custom domain (Phase 4), rather than deploying twice.
...
### Phase status
Phase 3R: content/revision work is complete (3R.1–3R.5 done, all issues found and fixed, customer completed a full page pass). **3R.6 (formal deployment approval) is explicitly NOT yet signed off** — customer wants to hold formal approval until the Jekyll conversion decision is resolved, since the plan is to deploy once to the custom domain from Jekyll's build output rather than deploy the current hand-coded site now and redeploy again after migrating. **Do not mark 3R fully complete or proceed toward Phase 4 until the customer gives explicit sign-off.**

### Next session priority
Jekyll conversion, scoped this session (Cowork) — see "Next session priority: Jekyll conversion" in the Quick Start section above for the full recommendation, estimate, and sequencing.
