...
- **Resume point:** Session 16 (Claude Code terminal) — two workflow/doc updates plus the first real Pass 2 of the image-selection workflow (Bicycle Maintenance Clamp), triggered by the customer dropping a new file into that project's folder between sessions.
  ...
  - **Pass 3 (derivative generation) and the actual page build are separate, not done for any of these three projects this session.**
...
- **Still open / needs customer input:**
  ...
  2. **Image review, 9 remaining folders** (Bicycle Maintenance Clamp, Cornhole Game Board, and Crayford Focuser now done) — whenever the customer is ready, in whatever order they choose: review each folder's numbered photos, hand back a hero pick + sequence. Pass 2/3 and the page builds are blocked on this per project.
...
6. **Verification:** confirmed via `git status` before starting each pass that the working tree was otherwise clean (no other untracked/unexpected changes). For Crayford Focuser specifically, checked both archive-target filenames didn't already exist in `_archive/` before moving (avoiding a silent overwrite of an unrelated same-named file), and confirmed the final folder listing matches the brief's target mapping file-for-file. Confirmed all three projects' final folder listings match their respective briefs exactly.

### Open items carried forward
- **About page (3.4) needs customer content** — not built, needs real copy decisions first.
- **Photo review needed per project, 9 remaining folders** (Bicycle Maintenance Clamp, Cornhole Game Board, and Crayford Focuser now done) — numbered/archived and ready; customer reviews each folder's numbered photos and returns a hero pick + sequence whenever ready, in whatever order they want to tackle the rest. Pass 2/3 and the actual page builds are blocked on this per project.
