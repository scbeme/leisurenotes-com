...
- **Resume point:** Session 16 (Claude Code terminal) — two workflow/doc updates plus the first real Pass 2 of the image-selection workflow (Bicycle Maintenance Clamp), triggered by the customer dropping a new file into that project's folder between sessions.
  ...
  12. **Vertical Tool Cart — full sequence rebuild, resolving the discovery flagged earlier this session** (`projects/vertical-tool-cart/`): customer confirmed the situation was intentional — a full replacement of the old lower-res survivors with 15 higher-resolution originals, already Finder-moved into `_archive/` by the customer themselves before this pass. First formalized that state in git: `git add -A` on the project folder, which git correctly recognized as 17 renames (content-similarity detection) despite the physical move happening outside `git mv` — old numbered files → `_archive/` under their original filenames, nothing lost. Then applied the customer's supplied sequence (from a Finder screenshot of gallery order) to the 15 new unprefixed files: straight batch rename, no `hero-`, no collisions (none of the 15 had numeric prefixes yet, so every target name was free). `tool cart00003.png` (front view with ruler for scale) — confirmed absent from the customer's screenshot, confirmed with the customer directly — archived rather than sequenced. Final: `01`–`11` are the `tool cart0000N.png` photos in the customer's stated order; `12`–`14` are the three `tool cart CAD0000N.png` renders. All 15 files accounted for (14 sequenced + 1 archived).
  13. **Baby Doll Carriage — Pass 2 applied, resequence only** (`projects/baby-doll-carriage/`): all 7 files were already clean Pass 1 survivors, no new/corrupted files involved this time. Customer's sequence `03, 05, 01, 07, 06, 02, 04` — no `hero-` this time either, matching the Tablesaw Vertical Tenon Jig precedent (a plain permutation, not a hero-plus-rest sequence). The permutation decomposed into three independent cycles, each requiring temp-name staging: a 2-cycle (`01`↔`03`), a 3-cycle (`02`→`06`→`05`→`02`), and a second 2-cycle (`04`↔`07`) — all three run via `tmp-a-`/`tmp-b-`/`tmp-c-` intermediate names per the customer's own cycle decomposition. Final: `01`=example-3D, `02`=rendered (was the homepage thumbnail source), `03`=3D-hidden-line-view, `04`=top-view, `05`=side-view, `06`=end-view, `07`=exploded-view. All 7 files accounted for, nothing excluded.
  14. **Bug found and fixed: 9 of 12 live homepage thumbnails were broken** (`data/projects.json`). Caught while writing up Baby Doll Carriage's summary — its `thumb` field pointed to a file that no longer existed after the resequence. Audited all 12 and found this had been happening silently since the very first Pass 2 of the session (Bicycle Maintenance Clamp): whenever a Pass 2 renamed the specific file a project's homepage thumbnail pointed to, `data/projects.json` never got updated to follow it. Resolved all 9 to their current filenames; 2 of them (Mechanical Pinball Machine, Mantel Clock) needed a different photo entirely since the original thumb photo had itself been archived/excluded — repointed both to their project's new hero image instead, flagged here rather than picked silently. Verified all 12 thumb paths resolve now. Also added a step to the Pass 2 workflow doc so this doesn't happen again going forward.
  - **Pass 3 (derivative generation) and the actual page build are separate, not done for any of these eleven projects this session.**
  ...
  - **Customer double-checked the "2 remaining" claim mid-session** — confirmed via `git log` that neither Baby Doll Carriage nor Biplane Wooden Toy had a Pass 2 commit yet (only the original harvest and the Session 15 Pass 1 batch touched either folder), so the count was accurate. Worth remembering: when a customer questions a status claim, verify against actual repo history before restating it, rather than just re-asserting from memory.
...
- **Still open / needs customer input:**
  ...
  2. **Image review, 1 remaining folder** (all others now done) — only Biplane Wooden Toy left. Whenever the customer is ready: review the 3 numbered photos, hand back a hero pick + sequence.
...
- **First action for next session:** Run the new git-status startup check as usual. Get Biplane Wooden Toy's sequence from the customer to close out image review for all 12 remaining projects, then Pass 3 (derivative generation) and actual page builds can start. Cloudflare Web Analytics is live on all 3 built pages — worth a check of the dashboard for incoming beacon data.
