...
- **Resume point:** Session 13 (Claude Code terminal) redesigned the Foosball Table hero section, per a Cowork planning session earlier the same day. Implemented on `projects/foosball-table/index.html` only — the other 12 pages still wait on customer sign-off of this version before Phase 3 rollout.
  1. Replaced the old single-photo `.project-hero` + 4-column icon `.project-spec` bar with one combined two-column hero block (`.hero-grid`, ~50/50 split): left column = category label → `<h1>` → 2x2 spec grid (label/value only, no icons) → download button/note; right column = a fixed 4:3 image tile (`object-fit: cover`, `var(--color-page-bg)` letterbox fallback). Stacks to image-on-top, content-below below 640px.
  2. **Dropped the spec icons** (Session 10 decision, reversed this session per customer) — removed `.spec-icon` CSS and the 4 inline Tabler SVGs entirely. Confirmed no other page referenced `.spec-icon` before deleting.
  3. **Hero photo swap decision — flagged for customer confirmation:** reviewed all 32 gallery photos (the landscape 768×576 ones) as possible landscape banner candidates per the customer's going-forward landscape-first hero rule. **None were suitable** — all 32 are in-progress workshop/build/detail shots (clamps, jigs, drill press, parts on a bench), not composed shots of the finished table. The only "finished table" photo in the project is the current hero source (`foosball-table-IMG_4829-e1641488166393-225x300.png`, portrait 225×300); its plain (non-cropped) sibling `foosball-table-IMG_4829-768x576.png` looked like a landscape option but turned out to be the same photo rotated 90° into a landscape file frame, not a real crop — unusable as-is. **Decision made per the brief's documented fallback:** kept the portrait original as the hero source and leaned on the tile's letterbox/cover treatment rather than swapping to a gallery photo. No swap happened — flagging this explicitly since the brief anticipated one might.
  4. Produced a new cropped derivative `foosball-table-IMG_4829-e1641488166393-hero-224x168.png` (exact 4:3, cropped via `sips` from the 225×300 portrait original, vertically offset up slightly to keep the full top rod handles in frame) — this is the tile's `<img src>`. The original 225×300 file is untouched and is what the lightbox opens.
  5. **New site-wide lightbox component** (`assets/js/lightbox.js`, vanilla JS, no library): click-to-enlarge on the hero tile and every `.project-gallery` image, dimmed full-viewport overlay showing the image at native aspect ratio, fit-to-viewport, centered. Closes via click-outside, close button, or Escape. Hero tile is an `<a>` wrapping the cropped derivative `<img>`, with its `href` carrying the original uncropped photo's path — the lightbox JS reads `href` for anchors (preventing navigation) and `src` for plain gallery `<img>`s.
  - **Caveat carried forward (same as Session 12):** no visual/screenshot verification was possible — Chrome extension not connected in this sandbox. Verified by tracing the grid/aspect-ratio CSS by hand and reviewing the rendered image crop directly. Worth a live eyeball next time the site is viewed.
  - See Session 13 Summary below for full detail.
...
- **Still open / needs customer input:**
  1. **Foosball Table template still not locked (2D.6)** — this session's hero/spec redesign is an additional change on top of the Session 10 "feature-complete" flag and Session 12's headline/footer changes; still needs one explicit customer sign-off pass before Phase 3 (remaining 12 pages) starts.
  2. **Hero photo swap — no swap made, needs a look:** see item 3 above. Worth customer eyes on whether the portrait-crop-with-letterbox treatment reads well enough, or whether a future re-shoot/better source photo of the finished table is worth doing before this template locks.
- **First action for next session:** Continue toward Foosball Table template sign-off / Phase 3 kickoff. Live visual check of this session's hero redesign (see caveat above) and of Session 12's footer layout (still unverified live). Cloudflare Web Analytics is live on all 3 pages — worth a quick check of the Cloudflare dashboard for incoming beacon data once the site is next deployed/viewed.
