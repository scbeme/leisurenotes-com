...
- **Still open / needs customer input:**
  1. **Build time/Skill level values, image selection, and page content are all now done for all 13 projects, including Flag Display Case's hero photo** — Phase 3's "All 13 Project Pages" checklist section is fully complete, no open per-page questions remaining. Favicon (3.23) and the Crayford Focuser folder/slug mismatch are also now done. Remaining Phase 3 "Assets & Polish" items: sitemap.xml (3.22), mobile-responsiveness check (3.25), and explicit download-link verification (3.24) — image optimization (3.20) is effectively already satisfied by the WebP pipeline. Then Phase 3R (customer review of the complete site).
...
- **First action for next session:** The natural next step is a customer walkthrough of the complete site (all 13 pages, live, no open questions) before diving into Phase 3's remaining Assets & Polish items (sitemap.xml, mobile-responsiveness, download-link verification) or Phase 3R review — check with the customer on sequencing rather than assuming. Cloudflare Web Analytics is live on all 16 built pages — worth a check of the dashboard for incoming beacon data now that there's real content to look at traffic for.
...
- Crayford Focuser folder/slug — resolved this session (see item 11 above); folder and slug both now `crayford-focuser-1-25`, matching the locked sitemap; no longer open.
- Favicon (3.23) — resolved this session (see item 12 above); no longer open.
- **Phase 3 "Assets & Polish" checklist (3.20, 3.22, 3.24, 3.25 remaining)** — image optimization (3.20) is effectively satisfied by the WebP pipeline, favicon (3.23) is now done; sitemap.xml (3.22), download-link verification (3.24), and mobile responsiveness (3.25) haven't been done yet. Phase 3R (customer review of the complete site) is the phase after that.
- Favicon missing — Phase 3 item 3.23, not blocking
- Git PAT expires ~2027-07-27 — renewal reminder
- Monitor repo size — revisit GitHub Releases fallback at ~800MB (per-file trigger ~60-80MB, see website-design.md)
- WordPress stays live until Phase 5 approved
