...
- **Resume point:** Session 16 (Claude Code terminal) — two workflow/doc updates plus the first real Pass 2 of the image-selection workflow (Bicycle Maintenance Clamp), triggered by the customer dropping a new file into that project's folder between sessions.
  ...
  - **Pass 3 (derivative generation) and the actual page build are separate, not done for any of these ten projects this session.**
...
- **Still open / needs customer input:**
  ...
  2. **Image review, 2 remaining folders** (Bicycle Maintenance Clamp, Cornhole Game Board, Crayford Focuser, Fan Powered Toy Car, Fidget Spinner, Flag Display Case, Mantel Clock, Mechanical Pinball Machine, Tablesaw Vertical Tenon Jig, and Vertical Tool Cart now done) — only Baby Doll Carriage and Biplane Wooden Toy left. Whenever the customer is ready, in whatever order they choose: review each folder's numbered photos, hand back a hero pick + sequence.
...
- **First action for next session:** Run the new git-status startup check as usual. Continue image review for Baby Doll Carriage and Biplane Wooden Toy as the customer works through them — the last 2 of the 12 remaining projects. Cloudflare Web Analytics is live on all 3 built pages — worth a check of the dashboard for incoming beacon data.
