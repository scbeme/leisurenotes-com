...
## Last updated: 2026-07-31 | Session: 18 | Phase 3 complete — Phase 3R next
...
- **Still open / needs customer input:**
  1. **Two large download files worth a look before Phase 4 deployment**: Crayford Focuser's SketchUp CAD zip and Fidget Spinner's CAD zip are each ~77MB, at the documented "~60-80MB per-file" GitHub watch threshold. Not urgent, not blocking Phase 3R, but flagged so it doesn't get lost.
  2. **Two minor mobile design-preference items**, found during the 3.25 code review, worth a real-device look during Phase 3R rather than confirmed bugs: `.spec-grid` stays 2 columns at all screen widths (wraps to 2 lines on narrow phones, doesn't break); `.chip` filter buttons are ~28-30px tap height, under the ~44px recommended minimum (still fully usable).
