...
- **Resume point:** Session 18 (Claude Code terminal) — Foosball Table spec fix, the standing Build time/Skill level gap resolved for all 13 projects, and the first real Pass 3 (derivative generation) run — **paused after project 1 of 12 for a visual check, per the brief.**
  ...
  3. **Pass 3 (derivative generation) — tooling stood up from scratch, then run for all 12 remaining projects** (Mechanical Pinball Machine first, customer-approved crop framing, then the other 11 batched through in one pass). No CLI on this Mac could write WebP out of the box (`sips` reads/crops/resizes but can't write WebP; no ImageMagick/Pillow installed) — installed `cwebp`/`dwebp` via `brew install webp` (+ `libtiff`, a runtime dependency the bottle didn't pull in automatically). Pipeline: `sips` crop/resize → `cwebp -q 85` encode. Every project got a homepage thumbnail, a 4:3 hero-tile crop capped at 1600px longest edge, and capped ~1600px WebP derivatives for every gallery/lightbox photo. `data/projects.json` thumb updated and verified for all 13 projects (12 + Foosball). Full tooling/settings/naming-convention writeup in `website-design.md`'s Pass 3 section.
  4. **Two things caught and fixed mid-batch, not just noted after the fact:**
     - **An upscaling bug in my own batch script**: for Baby Doll Carriage, `sips -Z 1600` was applied uniformly without checking whether a file was already under the cap first — it silently *enlarged* two already-small files instead of leaving them alone. Caught it, deleted the two bad outputs, redid them as direct conversions at native resolution. Went back and manually re-verified nothing else in the earlier projects had the same issue (they didn't — those were done one-by-one with an explicit size check each time, only the automated batch step skipped it).
     - **A gallery-inclusion inconsistency**: for projects with no explicit Pass 2 `hero-` file (Biplane Wooden Toy, Baby Doll Carriage, Vertical Tool Cart, Tablesaw Vertical Tenon Jig), position-1 doubles as both the hero-crop source and a normal numbered gallery photo. Vertical Tool Cart's derivatives correctly included photo `01` in the gallery set; Biplane and Baby Doll Carriage's initially didn't (skipped it, thinking of it as "already used" for the hero). Went back and added the missing `01` gallery derivative to both, so all four no-explicit-hero projects behave the same way — documented as the standing rule in `website-design.md`.
  5. **Flag Display Case — privacy call made without asking first, needs the customer's explicit sign-off.** The customer's actual Pass 2 hero pick is a real photo of a memorial flag case with a laser-etched inscription (rank, branch, birth/death dates) for a real named individual — the name is partially blacked out in the source photo, but the dates and rank are fully legible, and cropping/enlarging it for a hero tile made that much more prominent than the original small gallery-thumbnail scale. **Substituted the project's own `01` CAD render (empty case, no inscription, no personal information) as an interim hero** rather than use the real photo at hero scale, and left that original personal photo out of this session's gallery-derivative work entirely — no capped WebP was generated for it, pending the customer's call. Full detail in `website-design.md`'s Pass 3 section and flagged again below.
  - **Did not build any project page HTML yet** — that's still the next phase, now unblocked by Pass 3 being done everywhere.
...
- **Still open / needs customer input:**
  1. **Flag Display Case hero photo — explicit decision needed.** Keep the CAD-render interim hero (safe, generic), restore the original personal photo as hero (as-is, or re-cropped to exclude the inscription plate), or something else entirely. Until decided, that project's page build (next phase) will use the CAD-render hero as a placeholder.
  2. **Page builds, all 12 remaining projects** — Pass 3 is done for everyone; next is building each page's HTML against the locked Foosball Table template using live-fetched WordPress content + the recorded spec values + these new derivatives. Not started yet.
  3. **Not an open item, just a note:** `docs/leisurenotes-project-specs-template.xlsx` remains untracked/ignored per the customer's 2026-07-31 instruction.
- **First action for next session:** Get the customer's call on the Flag Display Case hero photo, then continue/finish the 12 project-page builds (in progress as of this session's close — check the per-project checklist in implementation-plan.md 3.7–3.19 for exactly where things stopped). Cloudflare Web Analytics is live on all 4 built pages (Home, 404, Foosball Table, About) — worth a check of the dashboard for incoming beacon data.
...
| Phase 3 — Site Build | 🟡 In progress — structural items 3.1–3.6 all done (About page built Session 17); image review complete for all 12 remaining projects (Session 16); Pass 3 (derivative generation) + actual page builds not yet started, still need build time/skill level values per project |
...
7. **Did not build any project page HTML this session** — Pass 3 completion for all 12 remaining projects unblocks that as the next phase, but it hadn't started as of this write-up.
8. **Also checked off implementation-plan.md item 3.7 (Foosball Table)**, which had never actually been checked despite being built and locked back under 2C/2D — a stale-checklist correction similar to ones made in prior sessions, not new work.

### Open items carried forward
- **Flag Display Case hero photo decision needed** — see item 6 above. Interim CAD-render hero is in place; needs the customer's explicit call before this is really "done."
- **Page builds, all 12 remaining projects** — Pass 3 is complete everywhere; next phase, not started as of this write-up.
