...
- **Resume point:** Session 16 (Claude Code terminal) — two workflow/doc updates plus the first real Pass 2 of the image-selection workflow (Bicycle Maintenance Clamp), triggered by the customer dropping a new file into that project's folder between sessions.
  ...
  - **Pass 3 (derivative generation) and the actual page build are separate, not done for any of these four projects this session.**
...
- **Still open / needs customer input:**
  ...
  2. **Image review, 8 remaining folders** (Bicycle Maintenance Clamp, Cornhole Game Board, Crayford Focuser, and Fan Powered Toy Car now done) — whenever the customer is ready, in whatever order they choose: review each folder's numbered photos, hand back a hero pick + sequence. Pass 2/3 and the page builds are blocked on this per project.
...
7. **Workflow correction mid-session:** the customer flagged that combining `cd` with a git command in one Bash invocation (`cd projects/<slug> && git mv ...`) trips an unbypassable Claude Code security prompt every time, even for allowlisted patterns — the `cd` itself is what's flagged, regardless of the git command that follows. From Crayford Focuser's renames onward, switched to `git -C /Users/ai/projects/web/leisurenotes-com <command>` (or full relative paths from the repo root) for every git invocation in this workflow — renames, `add`, `status`, `commit`, `push` alike — so the existing `Bash(git mv *)` etc. allowlist rules actually apply without a manual approval each time.
8. **Verification:** confirmed via `git status` before starting each pass that the working tree was otherwise clean (no other untracked/unexpected changes). For Crayford Focuser and Fan Powered Toy Car specifically, checked each archive-target filename didn't already exist in `_archive/` before moving (avoiding a silent overwrite of an unrelated same-named file). Confirmed all four projects' final folder listings match their respective briefs exactly, file-for-file.

### Open items carried forward
- **About page (3.4) needs customer content** — not built, needs real copy decisions first.
- **Photo review needed per project, 8 remaining folders** (Bicycle Maintenance Clamp, Cornhole Game Board, Crayford Focuser, and Fan Powered Toy Car now done) — numbered/archived and ready; customer reviews each folder's numbered photos and returns a hero pick + sequence whenever ready, in whatever order they want to tackle the rest. Pass 2/3 and the actual page builds are blocked on this per project.
